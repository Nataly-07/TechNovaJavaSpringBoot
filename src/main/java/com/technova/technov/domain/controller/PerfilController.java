...
public class PerfilController {
    ...
    public String perfilCliente(Model model) {
        ...
        try {
            List<com.technova.technov.domain.dto.NotificacionDto> notificacionesNoLeidas = notificacionService
                    .listarPorUsuarioYLeida(usuarioId, false);
    ...
    @GetMapping("/empleado/notificaciones")
    public String notificacionesEmpleado(Model model) {
        UsuarioDto usuario = securityUtil.getUsuarioAutenticado().orElse(null);
        ...
        List<NotificacionDto> notificaciones = notificacionService.listarPorUsuario(usuario.getId(), false);
        model.addAttribute("usuario", usuario);
    ...
    public String atencionCliente(Model model) {
        // Inicializar todas las variables con valores por defecto ANTES de cualquier
        // operación
        ...
        // Agregar atributos al modelo INMEDIATAMENTE para asegurar que siempre estén
        // presentes
        model.addAttribute("usuario", usuario);
        ...
        try {
            ...
            if (usuario.getId() != null) {
                try {
                    List<com.technova.technov.domain.dto.AtencionClienteDto> ticketsTemp = atencionClienteService
                            .listarPorUsuario(usuario.getId().intValue());
                    if (ticketsTemp != null) {
                        tickets = ticketsTemp;
                    }
                    model.addAttribute("tickets", tickets);
            ...
            if (usuario.getId() != null) {
                try {
                    List<com.technova.technov.domain.dto.MensajeDirectoDto> todosMensajes = mensajeDirectoService
                    ...
                    if (todosMensajes != null && !todosMensajes.isEmpty()) {
                        ...
                        for (com.technova.technov.domain.dto.MensajeDirectoDto mensaje : todosMensajes) {
                            try {
                                if (mensaje != null && mensaje.getConversationId() != null
                                        && !mensaje.getConversationId().isEmpty()) {
                                    com.technova.technov.domain.dto.MensajeDirectoDto existente = ultimosMensajes
                                            .get(mensaje.getConversationId());
                                    if (existente == null) {
    ...
    public String notificaciones(Model model) {
        ...
        try {
            if (usuario != null && usuario.getId() != null) {
                ...
                notificaciones = notificacionService.listarPorUsuario(usuario.getId(), false); // false = traer todas
                                                                                               // (leidas y no leidas)
    ...
    public String pedidosEmpleado(
        ...
        // Obtener usuarios y crear mapas ANTES de aplicar filtros (necesarios para
        // búsqueda general)
        List<UsuarioDto> todosUsuarios = usuarioService.listarUsuarios();
        ...
        try {
            ...
            if (categoria != null && !categoria.isEmpty()) {
                ...
                for (com.technova.technov.domain.dto.ProductoDto prod : todosProductos) {
                    if (prod.getId() != null && prod.getCaracteristica() != null
                            && prod.getCaracteristica().getCategoria() != null) {
        ...
        try {
            ...
            try {
                ...
                if (estado == null || estado.isEmpty() || "todas".equalsIgnoreCase(estado)) {
                    // Usar el servicio para obtener todos los tickets (asegura mapeo correcto y
                    // transacción)
                    ...
                    List<com.technova.technov.domain.dto.AtencionClienteDto> abiertos = atencionClienteService
                            .listarPorEstado("abierto");
                    List<com.technova.technov.domain.dto.AtencionClienteDto> enProceso = atencionClienteService
                            .listarPorEstado("en_proceso");
                    tickets = new java.util.ArrayList<>(abiertos);
                    tickets.addAll(enProceso);
                    // Ordenar por fecha descendente después de combinar
                    tickets.sort((a, b) -> {
                        if (a.getFechaConsulta() == null && b.getFechaConsulta() == null)
                            return 0;
                        if (a.getFechaConsulta() == null)
                            return 1;
                        if (b.getFechaConsulta() == null)
                            return -1;
                ...
                // Los tickets ya vienen ordenados del servicio, pero asegurémonos de que estén
                // ordenados
                                ...
                                .filter(t -> {
                                    ...
                                    if (t.getUsuarioId() != null && nombresUsuariosTemp.containsKey(t.getUsuarioId())) {
                                        ...
                                        if (nombreUsuario != null
                                                && nombreUsuario.toLowerCase().contains(busquedaLower)) {
                                            return true;
                                        }
                                    }
                                    return false;
            ...
            try {
                // Usar el mismo método que se usa para cargar los tickets para asegurar
                // consistencia
                List<com.technova.technov.domain.dto.AtencionClienteDto> todosTickets = atencionClienteRepository
                        .findAllByOrderByFechaConsultaDesc().stream()
                        .map(t -> {
                            com.technova.technov.domain.dto.AtencionClienteDto dto = modelMapper.map(t,
                                    com.technova.technov.domain.dto.AtencionClienteDto.class);
            ...
            try {
                ...
                for (UsuarioDto u : todosUsuarios) {
                    if (u.getId() != null) {
                        nombresUsuarios.put(u.getId().intValue(),
                                u.getName() != null ? u.getName() : "Usuario sin nombre");
                    }
                }
                model.addAttribute("nombresUsuarios", nombresUsuarios);
            } catch (Exception e) {
                            ...
                            .filter(r -> {
                                if ((r.getTitulo() != null && r.getTitulo().toLowerCase().contains(busquedaLower)) ||
                                        (r.getDescripcion() != null
                                                && r.getDescripcion().toLowerCase().contains(busquedaLower))) {
                                    return true;
                                }
    ...
    public String editarPerfilEmpleado(Model model) {
        ...
        // Cargar los datos más recientes de la base de datos para asegurar que se
        // muestren los cambios actualizados
    ...
    public String actualizarPerfilEmpleado(
        ...
        try {
            ...
            // Actualizar perfil usando el método correcto que actualiza phone, address y
            // password
    ...
    public String actualizarPerfil(
        ...
        // Si es modo "olvidé contraseña", la verificación de identidad ya se hizo en el
        // frontend
        ...
        // Si el usuario es admin o cliente, solo permitir actualizar teléfono,
        ...
        try {
            ...
            try {
                conversacionesTemp = todosMensajes.stream()
                        .filter(m -> m != null && m.getTipoRemitente() != null
                                && "admin".equalsIgnoreCase(m.getTipoRemitente()) && m.getRemitenteId() != null)
                        .collect(java.util.stream.Collectors.groupingBy(m -> m.getRemitenteId()));

                // Calcular mensajes no leídos por conversación y ordenar mensajes dentro de
                // cada conversación
                for (java.util.Map.Entry<Long, List<com.technova.technov.domain.dto.MensajeEmpleadoDto>> entry : conversacionesTemp
                        .entrySet()) {
                ...
                java.util.List<java.util.Map.Entry<Long, List<com.technova.technov.domain.dto.MensajeEmpleadoDto>>> conversacionesOrdenadas = new java.util.ArrayList<>(
                        conversacionesTemp.entrySet());
                ...
                conversacionesOrdenadas.sort((entry1, entry2) -> {
                    try {
                        ...
                        if (entry1.getValue() != null && !entry1.getValue().isEmpty()) {
                            com.technova.technov.domain.dto.MensajeEmpleadoDto ultimoMensaje1 = entry1.getValue()
                                    .get(0); // Ya está ordenado, el primero es el más reciente
                        ...
                        if (entry2.getValue() != null && !entry2.getValue().isEmpty()) {
                            com.technova.technov.domain.dto.MensajeEmpleadoDto ultimoMensaje2 = entry2.getValue()
                                    .get(0); // Ya está ordenado, el primero es el más reciente
                        ...
                        if (fecha1 == null && fecha2 == null)
                            return 0;
                        if (fecha1 == null)
                            return 1; // Sin fecha va al final
                        if (fecha2 == null)
                            return -1; // Sin fecha va al final
            ...
            // Si hay un ID de conversación (ID del admin), cargar todos los mensajes de esa
            // conversación
